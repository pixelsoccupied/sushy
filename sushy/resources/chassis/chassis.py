...
class Chassis(base.ResourceBase):
    ...
    power_data = base.Field('Power', required=False)
    """Raw power data when chassis is retrieved with $expand"""

    thermal_data = base.Field('Thermal', required=False) 
    ...
    def _get_expanded_data(self, field_name):
        ...
        # If expanded data only contains a reference (@odata.id), treat as unexpanded
        if (expanded_data and isinstance(expanded_data, dict) and 
            list(expanded_data.keys()) == ['@odata.id']):
            return None
